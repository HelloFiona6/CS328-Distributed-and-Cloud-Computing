...
int main(int argc, char *argv[])
{
  ...
  int padding_per_proc = padding / cal_size;
  ...
  double a_rcv[padding_per_proc][MAT_SIZE];
  // double b_rcv[MAT_SIZE][MAT_SIZE];
  double c_cal[padding_per_proc][MAT_SIZE];
  ...
  if (rank == 0)
  {
    ...
    // 给a_pad补齐0
    for (int i = MAT_SIZE; i < padding; i++)
    {
      for (int j = 0; j < MAT_SIZE; j++)
      {
        a_pad[i][j] = 0;
  ...
  // a: partial matrix
  MPI_Scatter(&a_pad, padding_per_proc * MAT_SIZE, MPI_DOUBLE, &a_rcv, padding_per_proc * MAT_SIZE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  // b: whole matrix
  MPI_Bcast(&b, MAT_SIZE*MAT_SIZE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  // printf("rank=%d, mpiSize=%d,\n a_rcv sized: %lu * %lu, b_rcv sized: %lu * %lu\n", rank, mpiSize, sizeof(a_rcv) / sizeof(a_rcv[0]), sizeof(a_rcv[0]) / sizeof(a_rcv[0][0]), sizeof(b) / sizeof(b[0]), sizeof(b[0]) / sizeof(b[0][0]));

  // print_matrix2D(padding / cal_size, MAT_SIZE, a_rcv, "rcv a");
  // print_matrix2D(MAT_SIZE, MAT_SIZE, b, "rcv b");

  /* Compute its own piece */
  for (int i = 0; i < padding_per_proc; i++)
  {
    for (int j = 0; j < MAT_SIZE; j++)
    {
      c_cal[i][j] = 0;
      for (int k = 0; k < MAT_SIZE; k++)
      {
        c_cal[i][j] += a_rcv[i][k] * b[k][j];
      }
    }
  }

  // print_matrix2D(padding_per_proc, MAT_SIZE, c_cal, "cal c");

  /* Gather */

  MPI_Gather(&c_cal, padding_per_proc * MAT_SIZE, MPI_DOUBLE, &c_pad, padding_per_proc * MAT_SIZE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  if (rank == 0)
  {
    ...
    // print_matrix2D(MAT_SIZE, MAT_SIZE, c, "C result");
    /* Measure finish time */
    ...
    // print_matrix2D(MAT_SIZE, MAT_SIZE, bfRes, "C result (brute force)");
    if (!checkRes(bfRes, c))
    {
      printf(
